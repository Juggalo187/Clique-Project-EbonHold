...
function Clique:GetBestRankForSpell(spellName)
    local bestRank = nil
    local bestLevel = -1   -- start at -1 so level 0 qualifies
    local playerLevel = UnitLevel("player") or 1
    local bookType = SpellBookFrame.bookType or BOOKTYPE_SPELL

    local numTabs = GetNumSpellTabs()
    for tabIdx = 1, numTabs do
        local name, texture, offset, numSpells = GetSpellTabInfo(tabIdx)
        for i = 1, numSpells do
            local slotIndex = offset + i
            local spellNameInBook, rank = GetSpellName(slotIndex, bookType)
            if spellNameInBook == spellName and rank and rank ~= "" then
                -- Ignore passive/racial passives
                if rank ~= L.PASSIVE and rank ~= L.RACIAL_PASSIVE then
                    local rankNum = tonumber(rank:match("%d+")) or 0
                    -- Estimate required level: Rank 1 → 0, Rank 2 → 10, Rank 3 → 20, ...
                    local level = (rankNum - 1) * 10
...
function Clique:SetAttribute(entry, frame)
	...
	elseif entry.type == "spell" then
		...
		local cast
		local spellName = entry.arg1
	
		if rank == "AUTO" then
			local bestRank = Clique:GetBestRankForSpell(spellName)
			if bestRank then
				cast = string.format("%s(%s)", spellName, bestRank)
			else
				cast = spellName   -- fallback if none found
			end
		elseif rank then
			if tonumber(rank) then
				cast = L.CAST_FORMAT:format(spellName, rank)
			else
				cast = string.format("%s(%s)", spellName, rank)
			end
		else
			cast = spellName
		end
		...
		-- These are optional (bag, slot, item, unit) – keep them for custom editor support
		frame:SetAttribute(entry.modifier.."bag"..button, entry.arg3)
		frame:SetAttribute(entry.modifier.."slot"..button, entry.arg4)
		frame:SetAttribute(entry.modifier.."item"..button, entry.arg5)
		if entry.arg6 then
			frame:SetAttribute(entry.modifier.."unit"..button, entry.arg6)
		end
